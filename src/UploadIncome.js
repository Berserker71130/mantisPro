...
const UploadIncome = () => {
  ...
  //   Function to process file and extract data
  const processFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setExcelData(json);
        console.log("Data loaded from Excel:", json);
      } catch (error) {
  ...
  //   Handler for the invisible file Input(used by browser and drag/drop)
  ...
  //   Handler for the final 'Continue import' action
  const handleImportData = () => {
    ...
    console.log("Attempting to send data to backend API:", excelData);

    // Simulating API call success
    setTimeout(() => {
      setUploadStatus("Success");
      alert(`Successfully processed ${excelData.length} rows of income data.`);
      handleClear();
    }, 2000);
  };
  ...
  //   DRAG AND DROP HANDLERS (NEW LOGIC)
  ...
  const StatusColor = (status) => {
    switch (status) {
      ...
      default:
        return "text-gray-600 border-green-300 bg-green-50";
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-8">
        ...
        <div className="mb-6 p-4 border-l-4 border-green-500 bg-green-50 text-green-700 rounded-lg">
          ...
          <p className="text-sm">
            Ensure your file contains required headers such as **Customer**,
            **Amount**, **Date**, **Product** and **Quantity**. Only **.xlsx**
            and **.xls** formats are accepted.
          </p>
        </div>
        ...
        <div
          ...
          onDrop={handleDrop}
        >
          {/* Invisible File input (Used by  both Browse and Drop) */}
          <input
          ...
          {fileName && uploadStatus !== "idle" ? (
            ...
            <>
              ...
              <label
                ...
                className="inline-flex items-center px-6 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 cursor-pointer transition duration-150"
              >
                Choose File
              </label>
            </>
          ...
          <button
            ...
            className={`px-8 py-3 rounded-lg font-bold text-white transition duration-300 ${
              excelData.length > 0 && uploadStatus !== "uploading"
                ? "bg-indigo-600 hoverP:bg-indigo-700 shadow-lg"
